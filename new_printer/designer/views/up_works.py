...
def stl_save(model,name,stl_type):
    #pdb.set_trace()
    chunks = ""
    ...
    data.append(chunks)
    data.append('--%s--\r\n' % boundary)
    http_url = adminer_website.toy_server_upload#'http://192.168.1.104:8888/file/upload'
    http_body = '\r\n'.join(data)
    req = urllib2.Request(http_url, data=http_body)
    req.add_header('Content-Type', 'multipart/form-data; boundary=%s' % boundary)
    req.add_header('User-Agent','Mozilla/5.0')
    req.add_header('Referer','%s'%adminer_website.toy_server_ip)#'http://192.168.1.104:8888/')
