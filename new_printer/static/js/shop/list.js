...
function paging_cb(e){
	...
	for(i=0;i<data.length;i++){
		_wrap.append(_node);
		$('.goods_img').eq(i).attr('src',data[i].goods_img);
		$('.goods_downloadNum').eq(i).text(data[i].goods_download_num);
		$('.goods_markNum').eq(i).text(data[i].goods_markNum);
		$('.goods_name').eq(i).text(data[i].goods_name);
		$('.goods_price').eq(i).text('￥' + data[i].goods_price);
		if(data[i].goods_mark){
			$('.list-mark-btn').addClass('active');
		}
	}
