...
function paging_cb(e){
	var _node = '<div class="list-box pr">' + $('.list-box').html() + '</div>',
		_wrap = $('.list-wrap'),
		data = JSON.parse(e).goods_list,
		goods_img = $('.goods_img'),
		goods_downloadNum = $('.goods_downloadNum'),
		goods_markNum = $('.goods_markNum'),
		goods_name = $('.goods_name'),
		goods_price = $('.goods_price'),
		mark_btn = $('.list-mark-btn');

	for(i=0;i<data.length;i++){
		_wrap.append(_node);
		goods_img.attr('src',data[i].goods_img);
		goods_downloadNum.text(data[i].goods_download_num);
		goods_markNum.text(data[i].goods_markNum);
		goods_name.text(data[i].goods_name);
		goods_price.text(data[i].goods_price);
		if(data[i].goods_mark){
			mark_btn.addClass('active');
		}
	}
