...
(function(){
	...
	function testNull(){	//测试是否为空的函数
		testInputs.each(function(index, el) {
			...
			if((_this.val()==="") || (_this.val()==0)){
				var thistext = _this.attr('form-null');
				if(!thistext){
					thistext = "输入不能为空";
				}
	...
	function testCont(element,content,reg){	//检测格式是否正确
		if(reg.test(content)){
			...
			var thisErr = element.attr('form-err');
			if(!thisErr){
				thisErr = "输入格式不正确";
			}
	...
	testInputs.each(function(index, el){	//输入框填入内容时，取消警告框
		$(this).change(function(event) {
			var _thischange=$(this);
			if((_thischange.val()==="") ||(_thischange.val()==0)){
				var thischangetext = _thischange.attr('form-null');
				warnShow(_thischange,thischangetext);
			}else if(!getType(_thischange)){
				var thiserrtext = _thischange.attr('form-err');
				warnShow(_thischange,thiserrtext);
			}else{
				warnhide(_thischange);
			}
		});
	});
